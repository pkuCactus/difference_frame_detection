...
### Task 1: 创建 .gitmodules 并注册所有 submodule
...
git submodule add https://github.com/rockchip-linux/rknpu2.git third-party/rknn-sdk
```
...
```bash
git add .gitmodules third-party/opencv third-party/yaml-cpp third-party/googletest third-party/rknn-sdk
...
删除以下内容（保留 `find_package(CURL REQUIRED)`）：
```cmake
...
将 `target_include_directories(difference_detection PRIVATE ...)` 从：
```cmake
...
将 `target_link_libraries(difference_detection PRIVATE ...)` 从：
```cmake
...
- [ ] **Step 2: 为主可执行文件添加 OpenCV 构建依赖**

在 `add_executable(difference_detection src/main.cpp ${SOURCES})` 之后添加：
```cmake
...
- [ ] **Step 1: 修改 `find_package(GTest REQUIRED)` 为条件检查**
...
- [ ] **Step 2: 移除硬编码路径，更新 yaml-cpp 和 OpenCV 链接**

将 `target_include_directories(difference_detection_tests PRIVATE ...)` 从：
```cmake
target_include_directories(difference_detection_tests PRIVATE
...
将 `target_link_libraries(difference_detection_tests PRIVATE ...)` 从：
```cmake
target_link_libraries(difference_detection_tests PRIVATE
    ${OpenCV_LIBS}
    ${CURL_LIBRARIES}
    /home/jianzhong/miniconda3/lib/libyaml-cpp.so
    GTest::gtest
...
- [ ] **Step 3: 确保测试目标依赖 opencv_external**

在 `add_executable(difference_detection_tests ${TEST_SOURCES})` 之后添加：
```cmake
...
- [ ] **Step 1: 获取 NDK r25c 的真实 SHA-256**

```bash
curl -sL https://dl.google.com/android/repository/android-ndk-r25c-linux-x86_64.zip | sha256sum
```
记录输出的 hash 值（64 位十六进制字符串），用于下一步。

- [ ] **Step 2: 创建 DownloadNDK.cmake**
...
# SHA-256 hash for NDK r25c Linux x86_64
# 获取方式: curl -sL https://dl.google.com/android/repository/${ANDROID_NDK_ZIP} | sha256sum
set(ANDROID_NDK_SHA256 "PLACEHOLDER_REPLACE_WITH_REAL_HASH")
...
将 `PLACEHOLDER_REPLACE_WITH_REAL_HASH` 替换为 Step 1 获取的真实 hash。
...
- [ ] **Step 2: 替换原有的 `if(RK3566_PLATFORM)` 块**
